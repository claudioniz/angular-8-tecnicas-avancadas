...
export class FilmesService {
  ...
  listar(pagina: number, registrosPorPagina: number, texto: string, genero: string): Observable<Filme[]> {
    let httpParams = new HttpParams();
    httpParams = httpParams.set('_page', pagina.toString());
    httpParams = httpParams.set('_limit', registrosPorPagina.toString());
